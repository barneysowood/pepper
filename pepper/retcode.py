...
class PepperRetcode(object):
    ...
    def validate(self, options, result):
        ...
        if options.retcode_passthrough and (
            options.client.startswith("runner") or options.client.startswith("wheel")
        ):
            return self.validate_runner_wheel(result)
        return 0
        ...
        if isinstance(result, list):
            if isinstance(result[0], dict):
                ...
                retcodes = list(
                    ...
                    and minion[name].get("retcode") is not None
                )
    ...
    @staticmethod
    def validate_runner_wheel(result):
        """
        Validate result dictionary retcode values for
        runner and wheel returns.
